...
<document>
	...
	<#if FirstPage == "Facture d'">
        <parameter name="DATE_FACTURE" value="${Date_facture2!}"/>
    <#else>
	...
	<parameter name="FACTURE_NO" value="${FactureID}"/> 
	...
	<target medium="FACTURE_SD_ARCHIVE" pages="1-" number="01" type="orig"></target>

	
	<#if PJDEPARTMENT != "KPDF">
		<#assign directPrinters = ["M007", "M021", "M032", "M033", "M054", "M026", "MFSD"]>
		<#if directPrinters?seq_contains(PJDEPARTMENT)>
			<target medium="FACTURE_SD_DIRECT" pages="1-" number="01" type="orig">
			</target>
			
		<#else>
			
			<target medium="FACTURE_SD" pages="1-" number="01" type="orig">
			</target>
		</#if>
	</#if>
	<pagedefinitions>
